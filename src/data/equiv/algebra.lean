...
/- The proof would be much simpler with new style structure command. -/
/-- Two multiplicative isomorphisms are equal, if their coercions to functions are equal. -/
@[extensionality, to_additive]
lemma ext {f g : α ≃* β} (h : (f : α → β) = g) : f = g :=
begin
replace h : f.to_equiv = g.to_equiv := equiv.eq_of_to_fun_eq h,
cases f, cases g,
congr,
  { exact congr_arg equiv.to_fun h },
...
/- The proof would be much simpler with new style structure command. -/
/-- Two ring isomorphisms are equal, if their coercions to functions are equal. -/
@[extensionality] lemma ext {f g : α ≃r β} (h : (f : α → β) = g) : f = g :=
begin
replace h : f.to_equiv = g.to_equiv := equiv.eq_of_to_fun_eq h,
cases f, cases g,
congr,
...
@[to_additive] def aut (γ : Type*) [has_mul γ] := mul_equiv γ γ
...
@[to_additive] def aut (γ : Type*) [group γ] := mul_equiv γ γ
...
{ map_add' := λ x y, is_ring_hom.map_add f.1, .. f }
